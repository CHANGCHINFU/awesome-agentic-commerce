...
- [Truth Bear (GAUGE)](https://truthbear.co) - Official-source data signals for AI agents, each reading stated against the source agency's own published threshold ladder rather than a scale we invented — descriptive only: no forecast, no recommendation, no adjudication. 180+ signals from named public records (USGS/NOAA/NWS/EPA/SEC EDGAR/openFDA/EIA/BLS/IMF PortWatch and more). Every paid record ships a canonical sha256 record_hash the caller can recompute offline, with daily batch anchoring to Bitcoin via OpenTimestamps. x402 pay-per-call in USDC on Base + Solana — the buyer's own wallet settles and we hold no keys. No API key, no signup. [MCP server](https://www.npmjs.com/package/mcp-gauge-x402) available.
