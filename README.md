...
- [Truth Bear (GAUGE)](https://truthbear.co) - Verifiable official-source data signals for AI agents: 180+ signals (USGS/NOAA/SEC EDGAR/EPA and more) with multi-source cross-validation and a tamper-proof receipt anchored to Bitcoin via OpenTimestamps. x402 pay-per-call in USDC on Base + Solana, no API key. [MCP server](https://www.npmjs.com/package/mcp-gauge-x402) available.
